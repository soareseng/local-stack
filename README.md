...
- `loki`: log storage
- `alloy`: central telemetry collector (logs, traces, metrics)
...
- Metrics: APIs expose `/metrics`; Alloy scrapes and remote-writes to Prometheus
- Logs: collected by Alloy and sent to Loki
- Traces: APIs send OTLP traces to Alloy; Alloy forwards to Jaeger
